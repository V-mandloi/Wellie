  ...
  return (
    ...
    <div className="bg-white p-6 rounded-lg shadow-md w-full md:w-[350px] flex flex-col justify-between border">
      ...
      <div className="flex items-center gap-4">
        ...
        <h3 className="text-2xl font-semibold text-hometext">{title}</h3>
      </div>
      ...
      {/* Supported Topics */}
      <p className="text-xl text-gray-content">
      ...
      {/* Description */}
      <p className="text-contetnt text-xl mt-2">{description}</p>

      {/* Note (if available) */}
      {note && <p className="text-lg text-gray-500 mt-1">{note}</p>}

      {/* Button */}
      <a
        href={buttonLink}
        className="block bg-buttonBg text-xl mt-4 py-2 px-4 rounded-md font-semibold text-center hover:bg-orange-100 transition"
      >
        {buttonText}
      </a>
    </div>
